# This script can assign iOS enrollment profile to a list of iPads
...
function Get-AuthToken {
    ...
    $CSVPath = Read-Host "Please enter the csv path"
    $DEPdevices = Import-Csv ($CSVPath)
    
    $devicemenu = @{}
    
    $devicemenu = $DEPdevices.Serialnumber
     
    Write-Host "$devicemenu"
    ...
    for ($i=1; $i -le $devicemenu.count; $i++){
    
    $DeviceSerialNumber = (@($devicemenu))[$i-1]
